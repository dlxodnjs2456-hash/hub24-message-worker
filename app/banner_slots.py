...
class BannerPurchase(BaseModel): plan:str
class BannerCreative(BaseModel):
    title:str|None=None
    image_url:str
    target_url:str|None=None
class BannerSlotSettings(BaseModel): banner_slot_count:int
...
def banner_slots(user=Depends(auth)):
    ...
    by={int(x['sort_order']):x for x in rows};items=[]
    for slot in range(1,count+1):
        b=by.get(slot);occupied=active(b);admin_banner=bool(b and not b.get('owner_user_id') and b.get('is_active') and b.get('image_url'))
        items.append({'slot':slot,'id':b.get('id') if b else None,'title':b.get('title') if b else None,'image_url':b.get('image_url') if b and (occupied or admin_banner) else '','target_url':b.get('target_url') if b and (occupied or admin_banner) else None,'is_active':bool(b and b.get('is_active') and (occupied or admin_banner)),'available':not occupied,'owned_by_me':bool(b and str(b.get('owner_user_id') or '')==str(user) and occupied),'expires_at':b.get('expires_at') if occupied else None,'is_lifetime':bool(b and b.get('is_lifetime') and occupied),'plan_code':b.get('plan_code') if occupied else None})
...
def purchase_banner(slot:int,p:BannerPurchase,user=Depends(auth)):
    ...
    try:return {'ok':True,'purchase':db.sb.rpc('npay_purchase_banner_slot',{'p_user':user,'p_slot':slot,'p_plan':p.plan.upper()}).execute().data}
...
def admin_banner_slot_settings(p:BannerSlotSettings,user=Depends(auth)):
    ...
    current=slot_count()
    if count<current:
        hidden=q('market_banners').select('*').gt('sort_order',count).lte('sort_order',6).execute().data or []
        protected=[b for b in hidden if active(b)]
        if protected:
            slots=','.join(str(b.get('sort_order')) for b in protected)
            raise HTTPException(409,f'ACTIVE_PAID_BANNER_EXISTS:{slots}')
    rows=q('market_settings').update({'banner_slot_count':count,'updated_at':now_iso()}).eq('id',1).execute().data or []
    try:q('admin_logs').insert({'admin_user_id':user,'action':'BANNER_SLOT_COUNT_CHANGE','target_type':'market_settings','target_id':'1','detail':{'from':current,'to':count}}).execute()
