...
def notification_read(nid:int,user=Depends(auth)):
    q('user_notifications').update({'is_read':True}).eq('id',nid).eq('user_id',user).execute()
    return {'ok':True}

@app.post('/v1/notifications/read-all')
def notifications_read_all(user=Depends(auth)):
    q('user_notifications').update({'is_read':True}).eq('user_id',user).eq('is_read',False).execute()
    return {'ok':True}
...
def add_trade_evidence(tid:int,p:EvidenceCreate,user=Depends(auth)):
    ...
    row=q('trade_dispute_evidence').insert({'trade_id':tid,'user_id':user,'evidence_type':p.evidence_type.upper(),'content':p.content.strip() if p.content else None,'file_url':p.file_url}).execute().data[0]
    return row
...
def resolve_withdrawal_v2(wid:int,p:WithdrawalResolveV2,user=Depends(auth)):
    ...
    if action=='PAID':
        q('withdrawal_requests').update({'status':'PAID','admin_reference':p.admin_reference,'admin_note':p.admin_note,'processed_at':now_iso(),'updated_at':now_iso()}).eq('id',wid).execute()
    elif action=='REJECT':
        db.sb.rpc('hub24_reject_withdrawal',{'p_request_id':wid}).execute()
        q('withdrawal_requests').update({'admin_note':p.admin_note,'processed_at':now_iso(),'updated_at':now_iso()}).eq('id',wid).execute()
    else: raise HTTPException(400,'INVALID_ACTION')
    audit(user,'WITHDRAWAL_'+action,'withdrawal',wid,{'reference':p.admin_reference,'note':p.admin_note})
    return {'ok':True,'status':'PAID' if action=='PAID' else 'REJECTED'}

@app.get('/v1/admin/logs')
def admin_logs(user=Depends(auth)):
    require_admin(user)
    return {'items':q('admin_logs').select('*').order('created_at',desc=True).limit(500).execute().data or []}
